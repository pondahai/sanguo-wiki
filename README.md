...
全書 120 回正文自動切分、人名自動連結,351 位人物各有條目——生平由本地 LLM
以 map-reduce 方式「全文閱讀」原著後生成,**每條事蹟都附回數出處,可回溯查證**。

- **Obsidian vault**(`vault/`):`[[人名]]` 點擊跳轉,graph view 看人物關係網
- **靜態網站**(475 頁):線上直接看 <https://pondahai.github.io/sanguo-wiki/>,
...
flowchart TD
    ...
    B -->|"build_wiki.py(不用LLM)<br>regex 切回·人名字典連結·統計"| C["vault/ 骨架<br>120回正文 + 351人物頁 + 索引"]
...
只有 183 個是真人)。先選 N≥30 新增 31 人,後來把門檻降到 N≥10 把候選池收滿,
人物頁達 351 人。

門檻要一次到底:`extract_facts.py` 是把**整回正文**餵給模型、只抽名單內的人,
120 回的閱讀成本跟名單長度幾乎無關——分兩批做等於付兩次 Map 的錢。
