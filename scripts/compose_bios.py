...
from build_wiki import VAULT
from extract_facts import FACTS, call_llm
...
def make_prompt(canon, aliases, facts):
    ...
    return (
        f"以下是從《三國演義》原文逐回抽取的、關於「{canon}」{alias_str}的全部事實清單,"
        "每條都附回數出處。請據此撰寫 wiki 條目,繁體中文(正體字,嚴禁出現任何簡體字)、Markdown,只輸出三節:\n\n"
        "### 生平概述\n(2-4 段,按時間順序綜述)\n\n"
        "### 重要事蹟\n(條列,每項務必註明回數,直接取自清單)\n\n"
        "### 人物關係\n(條列:主公、部下、親屬、盟友、對手,只列清單中有依據的)\n\n"
        "嚴格規則:\n"
        ...
        "5. 演義中有數組同名之人。若清單出現無法並存的矛盾——死後又有行動、分屬敵對陣營、"
        "同一時期身在兩地——那是同名的兩個人,不是劇情轉折。此時依時段與陣營分成兩段分別敘述,"
        "開頭寫明「演義中有二人同名」;嚴禁編造投降、復出、改仕等情節去把兩人接成一條故事線\n\n"
        f"=== 事實清單 ===\n{fact_lines}"
    )
...
def main():
    ...
    for canon in ranked:
        ...
        try:
            ...
            bio = fix_simplified(fix_links(bio))
