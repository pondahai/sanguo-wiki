...
from build_wiki import RAW, parse_chapters, reflow, build_alias_map
...
def make_prompt(num, title, text, present):
    names = "、".join(present)
    return (
        f"以下是《三國演義》第{num}回「{title}」的完整正文。請通讀全文,"
        ...
        "   (特別注意:正文泛稱「帝」「天子」「上」時,先確認指的是哪位皇帝,對不上的不寫)\n"
        "4. 演義中有數組同名之人(如吳將馬忠與蜀將馬忠)。只抽本回正文中該姓名所指者的言行,"
        "不得引入他處同名者的事跡;若本回無法確定是哪一位,該條寧可不寫\n"
        f"5. 只輸出 JSON,格式:{{\"人物名\": [\"事實\", ...], ...}},鍵只能是:{names}\n\n"
        f"=== 第{num}回正文 ===\n{text}"
    )
...
def main():
    ...
    for num, title, body in chapters:
        ...
        try:
            ...
            data = {k: [fix_simplified(x) for x in v] for k, v in data.items()}
